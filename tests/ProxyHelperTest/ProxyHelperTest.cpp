...
/*! @class ProxyHelperTest draupnir-lib/tests/ProxyHelperTest/FixedTupleMapTest.cpp
 *  @brief Test class for the ProxyHelper static object. */

class ProxyHelperTest final : public QObject
{
    Q_OBJECT

public:
    ProxyHelperTest() = default;
    ~ProxyHelperTest() final = default;
...
private slots:
    ...
    void testIsValidProxy() {
        ...
        for (auto data : validDatasetArray) {
            if (!ProxyHelper::isValidProxy(data.first)) {
                qWarning() << "Current data: "<<data;
            }
            QVERIFY(ProxyHelper::isValidProxy(data.first));
        }
    ...
    void testProxyFromString() {
        // Confirm that valid proxies are indeed valid:
        for (auto data : validDatasetArray) {
            const QNetworkProxy proxy = ProxyHelper::proxyFromString(data.first);
    ...
    void testStringFromProxy() {
        // Confirm that valid proxies are indeed valid:
        for (auto data : validDatasetArray) {
            const QString proxyString = ProxyHelper::stringFromProxy(data.second);
...
const QVector<QPair<QString,QNetworkProxy> > ProxyHelperTest::validDatasetArray = {
    // Http proxy
...
const QVector<QPair<QString,QNetworkProxy> > ProxyHelperTest::invalidDatasetArray = {
    // Unsopported schemas
...
QTEST_APPLESS_MAIN(ProxyHelperTest)

#include "ProxyHelperTest.moc"
