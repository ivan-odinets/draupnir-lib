...
class TuplePointerGuard
{
public:
    explicit TuplePointerGuard(Tuple& tuple) : m_tuple(tuple) {}
    ~TuplePointerGuard() {
    ...
    TuplePointerGuard(const TuplePointerGuard&) = delete;
    TuplePointerGuard& operator=(const TuplePointerGuard&) = delete;

private:
    Tuple& m_tuple;
};

/*! @class TemplateConstructorsTest tests/modules/utils/unit/template_detectors_test/TemplateConstructorsTest.cpp
...
private slots:
    void test_create_tuple_new() {
        ...
        TuplePointerGuard<TupleOne> oneGuard{one};
        // Check first
        ...
        TuplePointerGuard<TupleTwo> twoGuard{two};
        // Check first
        QVERIFY(std::get<0>(two) != nullptr);
        QCOMPARE((*std::get<0>(two)), QString{});
        // Check second
        QVERIFY(std::get<1>(two) != nullptr);
