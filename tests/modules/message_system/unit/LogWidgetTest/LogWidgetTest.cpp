...
private slots:
    ...
    void test_step_by_step_attaching() {
        ...
        bool isBriefDisplayed = defaultMessageFields & MessageField::Brief;
        QCOMPARE(widget->w_messagesListView->isMessageFieldDisplayed(MessageField::Brief), isBriefDisplayed);
        QCOMPARE(widget->w_messageListViewConfigMenu->isMessageFieldDisplayed(MessageField::Brief),isBriefDisplayed);
    }
    ...
    void test_message_fileds_display_behaviour() {
        ...
        // Change settings
        widget->w_messageListViewConfigMenu->getActionForField(MessageField::Brief)->trigger();

        // Check if settings were properly updated
        QCOMPARE(widget->w_messagesListView->isMessageFieldDisplayed(MessageField::Brief), !isBriefDisplayed);
        const auto settingsValue = settingsRegistry.get<Draupnir::Messages::Settings::LogWidget::DisplayedMessageFieldsSetting>();
        QCOMPARE(settingsValue | MessageField::Brief, defaultMessageFields);
    }
};
