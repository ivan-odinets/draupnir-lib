...
/*! @class FixedTupleMapTest tests/modules/containers/unit/fixed_tuple_map_test/FixedTupleMapTest.cpp
...
///@{
    ...
    /*! @brief Static constexpr RAW array with enum values. To be used as a template argument of fixed_tuple_map. */
    ...
    /*! @brief Static constexpr RAW array with some other set of enum values. To be used as a template argument of
     *         fixed_tuple_map. */
    static constexpr Type enumOtherRawArray[] = { First, Second, Third };

    /*! @brief Static constexpr RAW array with integers. To be used as a template argument of fixed_tuple_map. */
...
private slots:
    ...
    void test_initialization() {
        for (int i = 0; i < static_cast<int>(controlSizeOfRawArray); i++) {
            QCOMPARE(tupleMap.get<int>(enumRawArray[i]), 0);
            QCOMPARE(tupleMap.get<double*>(enumRawArray[i]), nullptr);
            QCOMPARE(tupleMap.get<QString>(enumRawArray[i]), QString{});
        }
    }
    ...
    void test_setters_and_getters() {
        ...
        // Checking
        for (int i = 0; i < static_cast<int>(controlSizeOfRawArray); i++) {
            QCOMPARE(tupleMap.get<int>(enumRawArray[i]), i*42);
            QCOMPARE(*tupleMap.get<double*>(enumRawArray[i]), i*M_PI);
            delete tupleMap.get<double*>(enumRawArray[i]); // We will not use this any further
            QCOMPARE(tupleMap.get<QString>(enumRawArray[i]), QString("testing fixed_tuple_map. %1").arg(i*42));
        }
        ...
        // Checking
        for (int i = 0; i < static_cast<int>(controlSizeOfRawArray); i++) {
            QCOMPARE(tupleMap.get<int>(enumRawArray[i]), (i*42 + 100));
            QCOMPARE(*tupleMap.get<double*>(enumRawArray[i]), i*M_E);
            delete tupleMap.get<double*>(enumRawArray[i]); // We will not use this any further
            QCOMPARE(tupleMap.get<QString>(enumRawArray[i]), QString("testing fixed_tuple_map. one more time %1").arg(i*42));
        }
    }
};
