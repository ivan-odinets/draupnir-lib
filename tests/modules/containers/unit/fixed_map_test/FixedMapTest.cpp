...
using namespace draupnir::containers;

/*! @class FixedMapTest tests/modules/containers/unit/fixed_map_test/FixedMapTest.cpp
 *  @brief Test class for testing fixed_map container.
 * @todo Add tests for runtime performance and comparing with std::map / std::unordered_map. */
...
private slots:
    ...
    void test_initialization() {
        // Check size
        QCOMPARE(decltype(enumIntegerMap)::keys_size,           controlSizeOfRawArray);
        QCOMPARE(decltype(enumDoublePointerMap)::keys_size,     controlSizeOfRawArray);
        QCOMPARE(decltype(integerIntegerMap)::keys_size,        integerValuesArraySize);

        // Check first and last keys
        QCOMPARE(enumIntegerMap.first_key(),     enumRawArray[0]);
        QCOMPARE(enumIntegerMap.last_key(),      enumRawArray[controlSizeOfRawArray-1]);
        QCOMPARE(enumQStringMap.first_key(),     enumRawArray[0]);
        QCOMPARE(enumQStringMap.last_key(),      enumRawArray[controlSizeOfRawArray-1]);
        QCOMPARE(integerIntegerMap.first_key(),  integerValuesArray[0]);
        QCOMPARE(integerIntegerMap.last_key(),   integerValuesArray[integerValuesArraySize-1]);
        ...
        for (int counter = 0; counter < static_cast<int>(controlSizeOfRawArray); counter++) {
            ...
            QCOMPARE(enumIntegerMapIterator->first,        enumRawArray[counter]);
            QCOMPARE(enumIntegerMapIterator->second,       0);

            QCOMPARE(enumDoublePointerMapIterator->first,  enumRawArray[counter]);
            QCOMPARE(enumDoublePointerMapIterator->second, nullptr);

            QCOMPARE(enumQStringMapIterator->first,        enumRawArray[counter]);
            QCOMPARE(enumQStringMapIterator->second,       QString());
        ...
        QCOMPARE(enumIntegerMapIterator,         enumIntegerMap.end());
        QCOMPARE(enumDoublePointerMapIterator,   enumDoublePointerMap.end());
        QCOMPARE(enumQStringMapIterator,         enumQStringMap.end());
        ...
        for (int counter = 0; counter < static_cast<int>(controlSizeOfRawArray); counter++) {
            // Check content of maps
            QCOMPARE(integerIntegerMapIterator->first,     integerValuesArray[counter]);
            QCOMPARE(integerIntegerMapIterator->second,    0);
    ...
    void test_getter_access() {
        ...
        for (int i = 0; i < static_cast<int>(controlSizeOfRawArray); i++) {
            // Integer
            QCOMPARE(enumIntegerMap.get(enumRawArray[i]), i);

            // Double pointer
            QVERIFY(enumDoublePointerMap.get(enumRawArray[i]) != nullptr);
            QCOMPARE(*enumDoublePointerMap.get(enumRawArray[i]), double{i*M_PI});

            // QString
            QCOMPARE(enumQStringMap.get(enumRawArray[i]), QString("TESTING ENUM MAP! %1").arg(i));
        }
    ...
    void test_operator_access() {
        ...
        for (int i = 0; i < static_cast<int>(controlSizeOfRawArray); i++) {
            // Integer
            QCOMPARE(enumIntegerMap[enumRawArray[i]], i*i);

            // Double pointer
            QVERIFY(enumDoublePointerMap[enumRawArray[i]] != nullptr);
            QCOMPARE(*enumDoublePointerMap[enumRawArray[i]], double{i*M_E});

            // QString
            QCOMPARE(enumQStringMap.get(enumRawArray[i]), QString("TESTING ENUM MAP in another way! %1").arg(i*3));
        }
    ...
    void test_clearing() {
        ...
        for (int i = 0; i < static_cast<int>(controlSizeOfRawArray); i++) {
            // Integer
            QCOMPARE(enumIntegerMap[enumRawArray[i]], 0);

            // Double pointer
            QCOMPARE(enumDoublePointerMap[enumRawArray[i]], nullptr);

            // QString
            QCOMPARE(enumQStringMap.get(enumRawArray[i]), QString());
        }
    }

    void test_the_same_keys() {
