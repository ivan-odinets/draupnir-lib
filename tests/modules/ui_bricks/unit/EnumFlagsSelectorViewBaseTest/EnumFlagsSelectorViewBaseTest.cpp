...
public:
    ...
    static constexpr integer displayedMasks[] = { OneThree, OneTwoThree, All };
...
public:
    explicit MyEnumFlagsMenu(MyEnumFlags initialState = 0, QWidget* parent = nullptr) :
        QMenu{parent},
        Draupnir::Ui::EnumFlagsMaskSelectorBase<MyEnumFlagsMenu, QAction, MyEnumFlags>{initialState}
    {}
 ...
 *  @brief Unit test for @ref Draupnir::UiBricks::EnumFlagsSelectorViewBase.
...
class EnumFlagsSelectorViewBaseTest : public QObject
{
    Q_OBJECT
private:
...
///@{
    ...
    static void verify_ui_elements_state(TestedSubclass* object) {
        verify_ui_elements_state(object, object->m_displayedEnumFlags);
    }
...
///@{
    ...
    static void test_constructor_impl() {
        ...
        QCOMPARE(allMask->m_displayedEnumFlags, MyEnumFlags::All);
    ...
    static void test_get_ui_element_impl(TestedSubclass* object) {
        ...
        QCOMPARE(object->getUiElement(MyEnum::One),
                 std::get<typename TestedSubclass::_FlagValueWrapper<MyEnum::One>>(object->m_uiElementsTuple).element);
        QCOMPARE(object->getUiElement(MyEnum::Two),
                 std::get<typename TestedSubclass::_FlagValueWrapper<MyEnum::Two>>(object->m_uiElementsTuple).element);
        QCOMPARE(object->getUiElement(MyEnum::Three),
                 std::get<typename TestedSubclass::_FlagValueWrapper<MyEnum::Three>>(object->m_uiElementsTuple).element);
        QCOMPARE(object->getUiElement(MyEnum::Four),
                 std::get<typename TestedSubclass::_FlagValueWrapper<MyEnum::Four>>(object->m_uiElementsTuple).element);
        // Check mask UiElement
        QCOMPARE(object->getUiElement(MyEnumFlags::OneThree),
                 std::get<typename TestedSubclass::_MaskValueWrapper<MyEnumFlags::OneThree>>(object->m_uiElementsTuple).element);
        QCOMPARE(object->getUiElement(MyEnumFlags::OneTwoThree),
                 std::get<typename TestedSubclass::_MaskValueWrapper<MyEnumFlags::OneTwoThree>>(object->m_uiElementsTuple).element);
        QCOMPARE(object->getUiElement(MyEnumFlags::All),
                 std::get<typename TestedSubclass::_MaskValueWrapper<MyEnumFlags::All>>(object->m_uiElementsTuple).element);
    }
...
private slots:
    void initTestCase() {
        ...
        menu->m_displayedEnumFlags = MyEnumFlags{MyEnumFlags::OneThree};
        ...
        menu->m_displayedEnumFlags = MyEnumFlags{MyEnumFlags::OneThree | MyEnum::Four};
        menu->template getFlagElement<MyEnum::One>()->setChecked(true);
        menu->template getFlagElement<MyEnum::Three>()->setChecked(true);
...
QTEST_MAIN(Draupnir::Ui::EnumFlagsSelectorViewBaseTest)

#include "EnumFlagsSelectorViewBaseTest.moc"
