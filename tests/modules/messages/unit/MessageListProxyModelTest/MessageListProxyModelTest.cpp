...
/*! @class MessageListProxyModelTest tests/modules/messages/unit/MessageListProxyModelTest.cpp
...
private slots:
    ...
    void init() {
        ...
        tester = new QAbstractItemModelTester{testedProxy, QAbstractItemModelTester::FailureReportingMode::QtTest};
    }

    void cleanup() {
        delete testedProxy; testedProxy = nullptr;
    }
    ...
    void test_setting_message_levels() {
        ...
        QCOMPARE(testedProxy->messageAt(0), debugOne);
        ...
        const auto msgView = testedProxy->messageAt(0);
        const MessageList actual{testedProxy->messageAt(0), testedProxy->messageAt(1) };

        QVERIFY(actual.contains(infoOne));
        QVERIFY(actual.contains(infoTwo));
    ...
    void test_setting_message_categories() {
        ...
        for (int row = 0; row < testedProxy->rowCount(); ++row) {
            QCOMPARE(testedProxy->messageAt(row)->type().category(), MessageCategory::Default);
        }
    ...
    void test_setting_message_fields() {
        ...
        auto item = testedProxy->underlyingMessageListModel()->itemAt(testedProxy->mapToSource(proxyIndex));
        QCOMPARE(testedProxy->data(proxyIndex,Qt::DecorationRole).value<QIcon>(), item.icon());
        QCOMPARE(testedProxy->data(proxyIndex,Qt::DisplayRole).toString(), QString{});
        ...
        QCOMPARE(testedProxy->isMessageViewItemFieldDisplayed(MessageViewItemField::DateTime), false);
        proxyIndex = testedProxy->index(0,0);
        item = testedProxy->underlyingMessageListModel()->itemAt(testedProxy->mapToSource(proxyIndex));
        QCOMPARE(testedProxy->data(proxyIndex,Qt::DecorationRole).value<QIcon>(), item.icon());
        QCOMPARE(testedProxy->data(proxyIndex,Qt::DisplayRole).toString(), item.brief());
        ...
        QCOMPARE(testedProxy->data(proxyIndex,Qt::DisplayRole).toString(),
                 item.getViewString(testedProxy->displayedMessageViewItemFieldsMask()));
        ...
        // Quicly check tooltips
        QCOMPARE(testedProxy->data(proxyIndex, Qt::ToolTipRole).toString(),item.what());
    }
    ...
    void test_setting_field_mask_notifies_views() {
        QSignalSpy spy{ testedProxy, &QAbstractItemModel::dataChanged };
...
QTEST_MAIN(MessageListProxyModelTest)
