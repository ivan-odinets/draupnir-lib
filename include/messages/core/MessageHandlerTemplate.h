...
public:
    using SettingsBundle = Draupnir::Settings::SettingsBundle<
        MessateTypeSettingsTrait<MessageTraits>...
    >;
    ...
    Notification::Type _notificationImpl(uint64_t messageId) {
        if (uint64_t(First::type) == messageId) {
            return m_settings.template get<MessateTypeSettingsTrait<First>>();
        }
...
private:
    ...
    inline void _setImpl(MessageType messageId, Notification::Type type) {
        if (First::type == messageId) {
            m_settings.template set<MessateTypeSettingsTrait<First>>(type);
            return;
        }
