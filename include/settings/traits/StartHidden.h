...
#ifndef STARTHIDDEN_H
#define STARTHIDDEN_H
...
/*! @var startHidden_settingsKey
 *  @brief String key for the "Start Hidden" setting in persistent storage.
...
inline constexpr const char startHidden_settingsKey[] = "startHidden";

/*! @typedef StartHidden
 *  @brief Concrete setting trait describing the "Start Hidden" option.
 *  @details This alias instantiates SettingTraitTemplate with:
 *           - Draupnir::Menus::StartHiddenMenuEntry — the associated menu entry trait;
 *           - bool — the underlying C++ value type;
 *           - startHidden_settingsKey — the persistent storage key;
...
using StartHidden = SettingTraitTemplate<
    Draupnir::Menus::StartHiddenMenuEntry,
    bool,
    startHidden_settingsKey,
    false
>;
