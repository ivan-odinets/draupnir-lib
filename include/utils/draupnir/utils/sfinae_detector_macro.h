...
#define DEFINE_EXTRACT_NESTED_TEMPLATE_TYPE_OR_VOID(NestedTemplateType, ...)            \
    template<class T, class = std::void_t<>>                                            \
    struct extract_##NestedTemplateType##_or_void { using type = void; };               \
                                                                                        \
    template<class T>                                                                   \
    struct extract_##NestedTemplateType##_or_void<                                      \
        T, std::void_t<typename T::NestedTemplateType<__VA_ARGS__>>                     \
    > { using type = typename T::NestedTemplateType<__VA_ARGS__>; };                    \
                                                                                        \
    template<class T>                                                                   \
    using extract_##NestedTemplateType##_or_void_t =                                    \
        typename extract_##NestedTemplateType##_or_void<T>::type;
...
/*! @def DEFINE_HAS_NESTED_TEMPLATE_BOOL(NestedTemplateBoolName, ...)
 *  @ingroup Utils
 *  @brief Generates `has_NestedTemplateBoolName<T>` and `has_NestedTemplateBoolName_v<T>` to detect presence of a `bool`-typed nested
 *         template static value `T::template NestedTemplateBoolName<...>`.
 *
 *  @details The detection succeeds only if the expression `T::template NestedTemplateBoolName<__VA_ARGS__>` is well-formed and has
 *           type `bool` (ignoring cv-qualifiers). */

#define DEFINE_HAS_NESTED_TEMPLATE_BOOL(NestedTemplateBoolName, ...)                    \
    template<class T, class = void>                                                     \
    struct has_##NestedTemplateBoolName : std::false_type {};                           \
                                                                                        \
    template<class T>                                                                   \
    struct has_##NestedTemplateBoolName<                                                \
    ...
    > : std::bool_constant<                                                             \
        ...
        > {};                                                                           \
                                                                                        \
    template<class T>                                                                   \
    inline static constexpr bool has_##NestedTemplateBoolName##_v =                     \
        has_##NestedTemplateBoolName<T>::value;

/*! @def DEFINE_EXTRACT_NESTED_TEMPLATE_BOOL_OR_FALSE(NestedTemplateBoolName, ...)
 *  @ingroup Utils
 *  @brief Generates a trait that reads `T::template NestedTemplateBoolName<...>` as `bool`, or yields `false` when unavailable.
 *
 *  @details The primary template inherits from `std::false_type`. The partial specialization is enabled when `T::template
 *           NestedTemplateBoolName<__VA_ARGS__>` is well-formed and then inherits from `std::bool_constant< value >`. A
 *           convenience variable `<NestedTemplateBoolName>_or_false_v<T>` is also provided. */

#define DEFINE_EXTRACT_NESTED_TEMPLATE_BOOL_OR_FALSE(NestedTemplateBoolName, ...)       \
    template<class T, class = void>                                                     \
    struct extract_##NestedTemplateBoolName##_or_false : std::false_type {};            \
                                                                                        \
    template<class T>                                                                   \
    struct extract_##NestedTemplateBoolName##_or_false<                                 \
        T, std::void_t<decltype(T::template NestedTemplateBoolName<__VA_ARGS__>)>       \
    > : std::bool_constant<                                                             \
        ...
        > {};                                                                           \
                                                                                        \
    template<class T>                                                                   \
    inline static constexpr bool extract_##NestedTemplateBoolName##_or_false_v =        \
        extract_##NestedTemplateBoolName##_or_false<T>::value;

/*! @def DEFINE_EXTRACT_NESTED_TEMPLATE_BOOL_OR_ASSERT(NestedTemplateBoolName, ...)
 *  @ingroup Utils
 *  @brief Generates a trait that reads `T::template NestedTemplateBoolName<...>` as `bool`, or produces a compile-time error if
 *         it does not exist or is of the wrong type.
 *
 *  @details The primary template always triggers a `static_assert`. The partial specialization is enabled when `T::template
 *           NestedTemplateBoolName<__VA_ARGS__>` exists and its type is `bool`, and provides a `value` static member plus
 *           `extract_NestedTemplateBoolName_or_assert_v<T>`. */

#define DEFINE_EXTRACT_NESTED_TEMPLATE_BOOL_OR_ASSERT(NestedTemplateBoolName, ...)      \
    template<class T, class = void>                                                     \
    struct extract_##NestedTemplateBoolName##_or_assert {                               \
        static_assert(sizeof(T) == 0,                                                   \
    ...
    > {                                                                                 \
        ...
        static constexpr bool value =                                                   \
            T::template NestedTemplateBoolName<__VA_ARGS__>;                            \
                                                                                        ...
                                                                                        \
    template<class T>                                                                   \
    inline static constexpr bool extract_##NestedTemplateBoolName##_or_assert_v =       \
        extract_##NestedTemplateBoolName##_or_assert<T>::value;
