 ...
 *
 * draupnir-lib
 * Copyright (C) 2025-2026z Ivan Odinets <i_odinets@protonmail.com>
 *
...
void MessageDisplayDialog::addMessageList(const MessageList& messages)
{
#ifndef QT_NO_DEBUG
    // Just to be sure that no nullptrs are here
    for (const auto& message : messages) {
        Q_ASSERT_X(message, "MessageDisplayDialog::addMessageList",
            "One of the provided MessagePtr is nullptr.");
    }
#endif // QT_NO_DEBUG
...
void MessageDisplayDialog::addMessage(const MessagePtr& message)
{
    Q_ASSERT_X(message, "MessageDisplayDialog::addMessage",
        "Provided MessagePtr pointer is nullptr.");
...
void MessageDisplayDialog::addMessage(MessagePtr&& message)
{
    Q_ASSERT_X(message, "MessageDisplayDialog::addMessage",
        "Provided MessagePtr pointer is nullptr.");
    MessageDisplayWidget* widget = new MessageDisplayWidget;
    widget->showMessage(std::move(message));
