...
void MessageListModel::append(Message* message)
{
    ...
    append(new MessageViewItem{message});
}

void MessageListModel::append(MessageViewItem* message)
{
...
void MessageListModel::append(const QList<Message*>& messages) {
    ...
    QList<MessageViewItem*> result;
    result.reserve(messages.count());
    for (Message* message : messages)
        result.append(new MessageViewItem{message});
    append(result);
}

void MessageListModel::append(const QList<MessageViewItem*>& messages)
{
    if (messages.isEmpty())
        return;
...
#ifndef QT_NO_DEBUG
    // Just to be sure that no nullptrs are here
    for (MessageViewItem* message : messages) {
        Q_ASSERT_X(message, "MessageListModel::append", "One of the provided MessageViewItem* is nullptr.");
    }
#endif // QT_NO_DEBUG
...
QVariant MessageListModel::data(const QModelIndex& index, int role) const
{
    ...
    const auto* const message = static_cast<MessageViewItem*>(index.internalPointer());
